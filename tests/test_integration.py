...
from tests.common import request_headers
...
class InstalledShowcaseIntegrationTests(unittest.TestCase):
    ...
    def headers(
        *,
        ip="192.168.10.10",
        device="browser-customer-001",
        hour=10,
        user_agent="UHAMKAMartBrowser/1.0",
    ):
    ...
    def test_storefront_and_runtime_are_packaged_with_framework(self):
        ...
        self.assertEqual(bootstrap["module"]["distribution"], "ratf")
        self.assertEqual(bootstrap["module"]["policy"]["name"], DEMO_POLICY_NAME)
    ...
    def test_allow_verify_block_and_replay_are_visible_to_client(self):
        ...
        changed = self.client.post(
            ...
            headers=self.headers(
                ip="103.77.14.90",
                device="browser-customer-001",
                user_agent="AutomationClient/3.0",
            ),
        )
        self.assertEqual(changed.status_code, 401)
        self.assertEqual(changed.headers["X-RATF-Decision"], "verify")
        self.assertEqual(float(changed.headers["X-RATF-Score"]), 0.7075)
